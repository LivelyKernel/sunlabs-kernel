...
TestCase.subclass('Tests.CouchdbTest.CouchdbTest', {
	...
	testDBConnection: function() {
		...
		this.assertEqual(res.db_name,this.dbName,"info broken");
	},
	...
	testJens: function() {
		...
		this.assertEqual(o5.r1.r.test , o5.r2.r.test,"ident not preserved"); 
	},
	...
	testNaming: function(){
		try{
			...
			this.assertEqual(o1._clazz,o2._clazz);
			this.assert(false);
		}
		catch(e){
			this.assertEqual(e.error, "doc_validation");
		}
	},
	...
	testNaming2: function(){
		...
		this.cdb.save(o1);
		var o2 = this.cdb.open(o1._id);
		this.assertEqual(o1.$clazz,o2.$clazz);
...
MorphTestCase.subclass("Tests.CouchdbTest.JsonRestorer",{
	...
	testSerializationSimpleMember:function() {
		...
		this.saveAndLoad();
		this.assertEqual(this.morph.test,this.restoredMorph.test);
		this.assertEqual(this.morph.getType(),this.restoredMorph.getType());
		this.assertEqual(this.morph.id(),this.restoredMorph.id());
	},
	
	testSerializationMorphsMember: function() {
		this.saveAndLoad();
		this.assert("label" in this.restoredMorph,"No Label there");
		this.assertEqual(this.restoredMorph.label.textString,this.nameString);
	},
	...
	testEscape:function() {
		...
		this.saveAndLoad();
		this.assertEqual("natoll",this.morph._natoll, "real Problem");
		this.assertEqual("natoll",this.restoredMorph._natoll,"propertie starting with _");
		this.assertEqual("minus",this.restoredMorph["-minus"],"propertie already starting with -");
		this.assertEqual(this.morph.aObj.key,this.restoredMorph.aObj.key, "ref/id problems with value");
		this.assertEqual(this.restoredMorph.aObj,this.restoredMorph._asWell, "ref/id problems");
	},
	
	testSerializationMorphsMemberId: function() {
		this.saveAndLoad();
		this.assertEqual(this.morph.label.id(),this.restoredMorph.label.id());
	},
	
	testSerializationSubmoprh: function() {
		this.saveAndLoad();
		this.assertEqual(this.morph.submorphs.length,this.restoredMorph.submorphs.length,"wrong submorphs.length");
	...
	testArraySerialization: function() {
		...
		this.saveAndLoad();
		this.assertEqual(this.morph.myArray.length, this.restoredMorph.myArray.length);
		for(var i = 0; i<this.morph.myArray;i++){
			this.assertEqual(this.morph.myArray[i],this.restoredMorph.myArray[i]);
		}
	},
	...
	testPlainObj: function() {
		...
		this.saveAndLoad();
		this.assertEqual(this.morph.pojso.bar, this.restoredMorph.pojso.bar);
		this.assertEqual(this.morph.pojso.foo, this.restoredMorph.pojso.foo);
	},
	
	testObjWithMorph: function() {
		this.morph.objWithM = {supi:"toll", more:this.morph.label};
		this.saveAndLoad();
		this.assertEqual(this.morph.objWithM.supi,this.restoredMorph.objWithM.supi);
		this.assertEqual(this.morph.objWithM.more.id(),this.restoredMorph.objWithM.more.id());
	},
	
	testArrayWithMorph: function() {
		this.morph.objWithM = ["toll", this.morph.label];
		this.saveAndLoad();
		this.assertEqual(this.morph.objWithM[0],this.restoredMorph.objWithM[0]);
		this.assertEqual(this.morph.objWithM[1].id(),this.restoredMorph.objWithM[1].id());
	},
	
	testObjeWithoutId: function() {
		this.morph.myColor = new Color(1,0,0.5);
		this.saveAndLoad();
		this.assertEqual(this.morph.myColor.r, this.restoredMorph.myColor.r);
		this.assertEqual(this.morph.myColor.g, this.restoredMorph.myColor.g);
		this.assertEqual(this.morph.myColor.b, this.restoredMorph.myColor.b);
	},
	...
	testHowItLooksLike: function() {
		...
		this.assertEqual(this.morph.getBorderWidth(),bowi);
		...
		this.assertEqual(this.restoredMorph.label.getPosition(),this.morph.label.getPosition());
	},
	
	assertSameColor: function(col1,col2) {
		this.assertEqual(col1.r,col2.r);
		this.assertEqual(col1.g,col2.g);
		this.assertEqual(col1.b,col2.b);
	},
	...
	testTransformator: function() {
		...
		this.saveAndLoad();
		this.assertEqual(this.morph.getScale(), this.restoredMorph.getScale(),"scale wrong");
		this.assertEqual(this.morph.getRotation(), this.restoredMorph.getRotation(),"Rotation wrong");
	},
	...
	testFont: function() {
		...
		this.saveAndLoad();
		this.assertEqual(this.morph.getFontFamily(), this.restoredMorph.getFontFamily(),"FontFamily wrong");
		this.assertEqual(this.morph.getFontSize(), this.restoredMorph.getFontSize(),"FontSize wrong");
	},
	
	testPointerEventsNone: function() {
		this.assertEqual(this.morph.label.getTrait("pointer-events"),"none");
		this.saveAndLoad();
		this.assertEqual(this.restoredMorphFromSubmorph.getTrait("pointer-events"),"none");
	},
...
TestCase.subclass('Tests.CouchdbTest.ReferenceSerializationTest', {
	...
	testSameObject: function() {
		...
		this.assertEqual(parentObj.r1, parentObj.r2);
		...
		restorer.patchRefs();	
		this.assertEqual(restored.r1.val, restored.r2.val,"Something realy went wrong");
		this.assertEqual(restored.r1, restored.r2,"no identity");
	},
	...
	testPathInResut: function() {
		...
		this.assertEqual(jso.r2.$ref,"$.r1");
	...
	testCycles: function() {
		...
		this.assertEqual(obj.key1,restored.key1, "Simple values not correct");
		this.assertEqual(restored.that, restored,"Cycle not correctly restored");
	},
	...
	testCycles2: function() {
		...
		other.r2 = obj;
		obj.r1 = other;
		this.assertEqual(other.r2.r1, other,"cycle starting at 'other' does not work");
		this.assertEqual(obj.r1.r2, obj,"cycle starting at 'obj' does not work");
		...
		this.assertEqual(restored.r1.r2, restored,"cycle starting at 'restored' does not work");
	}
...
MorphTestCase.subclass('Tests.CouchdbTest.ShapeJson', {
	...
	testRect: function() {
		...
		for(var i = 0; i<shp.rawNode.attributes.length; i++){
			...
			this.assertEqual(newSpec[key],val);
		}
	},
...
TestCase.subclass('Tests.CouchdbTest.OtherTest', {
	testKlassGeneration: function() {
        ...
        if (klass) {
            var morph = new klass();
            this.assertEqual(morph.getType(),type);
        } else {
	...
	testArrayClean: function() {
		...
		this.assertEqual(resAry.length,2);
		this.assertEqual(resAry[0],"a");
		this.assertEqual(resAry[1],"b");
		...
		resAry = restorer.cleanArray(myAry);
		this.assertEqual(resAry.length,3);
		this.assertEqual(resAry[0],"a");
		this.assertEqual(resAry[2],"b");
	},
	...
	testEvalRef:function() {
		...
		parentObj.r2 = childObj;
		parentObj.r1.val = "e";
		this.assertEqual(parentObj.r1.val,parentObj.r2.val);
	...
	testAddMethod: function() {
		...
		this.assertEqual(TestSubClass.prototype.foo(), "TestSubClass::foo", "wrong foo in TestSubClass proto");
		...
		this.assertEqual(TestSubSubClass.prototype.bar(), "TestSubSubClass::bar", "wrong bar in TestSubSubClass proto");
		
		var testSubObj = new TestSubClass();
		this.assertEqual(testSubObj.foo(), "TestSubClass::foo", "wrong foo in TestSubClass");
		
		var testSubSubObj = new TestSubSubClass();
		this.assertEqual(testSubSubObj.foo(), "TestSubClass::foo", "wrong foo in TestSubSubClass");
