...
Object.subclass('WikiNetworkAnalyzer', {
	...
	return strings.uniq().collect(function(ea) { return new URL(ea) });
},
...
buildDependencyList: function() {
	// all words for each --> get dependencies, add to list
},
findOrCreateProxiesForLinksIn: function(worldDocument) {
	return this.extractLinksFromDocument(worldDocument).collect(function(linkUrl) {
		var wp = this.worldProxies.detect(function(proxy) { return proxy.getURL() === linkUrl }) ||
						new WikiWorldProxy(linkUrl, this.url);
		if (!this.worldProxies.include(wp)) this.worldProxies.push(wp);
		return wp;
	}, this);
},
fetchLinksOfWorld: function(worldProxy, optCb) {
...
addLinksOfWorld: function(worldProxy, worldDoc) {
	...
	worldProxy.setLinkProxies(this.findOrCreateProxiesForLinksIn(worldDoc));
},

});
Object.subclass('WikiWorldProxy', {
...
findLinksToOtherWorlds: function() {
	var analyzer = new WikiNetworkAnalyzer(this.repourl);
	analyzer.fetchLinksOfWorld(this);
},
getURL: function() { return this.url  },
setLinkProxies: function(worldProxies) { this.linkProxies =  worldProxies },
getLinkProxies: function(worldProxies) { return this.linkProxies },
getNamesOfLinkedWorlds: function(worldProxies) {
	return this.getLinkProxies.collect(function(ea) { return ea.localName() });
},
localName: function() {
